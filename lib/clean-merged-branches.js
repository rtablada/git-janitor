...
export default async function cleanMergedBranches({ defaultBranchName, dryRun }) {
  ...
  let deleteableBranchResults = await Promise.all(branchNames.map(async (b) => await getDeleteBranchResult(b)));
